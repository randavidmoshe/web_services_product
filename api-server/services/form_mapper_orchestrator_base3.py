...
# Module-level shared connection pool for orchestrator
_orchestrator_redis_pool = redis_lib.ConnectionPool(
...
class FormMapperOrchestrator:
    ...
    def __init__(self, redis_client_or_db, db_session=None):
        if hasattr(redis_client_or_db, 'query'):
            ...
            self.redis = redis_lib.Redis(connection_pool=_orchestrator_redis_pool)
        else:
    ...
    def create_session(self, session_id=None, user_id=None, company_id=None, network_id=None,
        ...
        if form_route_id and not skip_cleanup:
    ...
    def get_session(self, session_id: str) -> Optional[Dict]:
        ...
        bool_fields = ["in_recovery_mode", "pending_validation_error_recovery", "no_more_paths"]
        ...
        for k, v in data.items():
            ...
            elif ks in bool_fields:
                decoded[ks] = vs.lower() == "true" if vs else False
            else:
                decoded[ks] = vs
        return decoded
    ...
    def _push_agent_task(self, session_id: str, task_type: str, payload: Dict) -> Dict:
        ...
        if not user_id:
            logger.error(f"[Orchestrator] No user_id for session {session_id}, cannot push task")
    ...
    def _execute_next_step(self, session_id: str) -> Dict:
        ...
        if current_index >= len(all_steps):
            ...
            # It looks like This block never executes - can be removed ?
            # Check if we just finished verify recovery steps
            #if session.get("in_verify_recovery_mode"):
            #    logger.info(
            #        f"[Orchestrator] Verify recovery steps complete, getting DOM for regenerate_verify_steps")
            #    self.update_session(session_id,
            #                        {"in_verify_recovery_mode": False, "pending_verify_regenerate": True})
            #    self.transition_to(session_id, MapperState.DOM_CHANGE_GETTING_SCREENSHOT)
            #   task = self._push_agent_task(session_id, "form_mapper_get_screenshot",
            #                                 {"encode_base64": True, "save_to_folder": False,
    ...
    def handle_step_result(self, session_id: str, result: Dict) -> Dict:
        ...
        # Build force_regenerate info
        force_info_str = ""
        if step.get('force_regenerate'):
            force_info_str += ", force_regenerate=true"
        if step.get('force_regenerate_verify'):
            force_info_str += ", force_regenerate_verify=true"
    ...
    def _handle_step_failure(self, session_id: str, result: Dict) -> Dict:
        ...
        if consecutive_failures >= max_retries:
            ...
            log = self._get_logger(session_id)
            log.error(f"!!!!! Max consecutive failures ({max_retries}) reached", category="error",
                      consecutive_failures=consecutive_failures, max_retries=max_retries,
                      failed_step=step.get("description", "")[:50])
            self._push_agent_task(session_id, "form_mapper_log_bug", {
                "bug_description": f"Recovery failed after {max_retries} attempts: {step.get('description', '')[:50]}",
                "log_level": "error",
                "screenshot": True,
                "bug_type": "max_retries_exceeded"
            })
    ...
    def handle_step_failure_recovery_result(self, session_id: str, result: Dict) -> Dict:
        ...
        if result.get("page_error_detected"):
            error_type = result.get("error_type", "unknown")
            logger.error(f"[Orchestrator] Page error detected: {error_type} - failing session")
            log = self._get_logger(session_id)
            log.error(f"!!!!!! Page error detected: {error_type}", category="error", error_type=error_type)
            self._push_agent_task(session_id, "form_mapper_log_bug", {
                "bug_description": f"Page error: {error_type}",
                "log_level": "error",
                "screenshot": True,
                "bug_type": "page_error"
            })
    ...
    def handle_regenerate_steps_result(self, session_id: str, result: Dict) -> Dict:
        ...
        if result.get("page_error_detected"):
            error_type = result.get("error_type", "unknown")
            logger.error(f"[Orchestrator] Page error detected during verification: {error_type} - failing session")
            log = self._get_logger(session_id)
            log.error(f"!!!!! Page error detected during verification: {error_type}", category="error", error_type=error_type)
            self._push_agent_task(session_id, "form_mapper_log_bug", {
                "bug_description": f"Page error during verification: {error_type}",
                "log_level": "error",
                "screenshot": True,
                "bug_type": "page_error"
            })
    ...
    def handle_regenerate_verify_steps_result(self, session_id: str, result: Dict) -> Dict:
        ...
        # Check if page error detected - log bug with screenshot, then fail session
        if result.get("page_error_detected"):
            error_type = result.get("error_type", "unknown")
            logger.error(f"[Orchestrator] Page error detected in verify: {error_type} - failing session")
            log = self._get_logger(session_id)
            log.error(f"!!!!! Page error detected in verify: {error_type}", category="error", error_type=error_type)
            self._push_agent_task(session_id, "form_mapper_log_bug", {
                "bug_description": f"Page error in verify: {error_type}",
                "log_level": "error",
                "screenshot": True,
                "bug_type": "page_error"
            })
    ...
    def handle_continue_mapping_evaluation_result(self, session_id: str, result: Dict) -> Dict:
        """Handle result from AI path evaluation for continue mapping flow"""
        session = self.get_session(session_id)
        if not session:
            return {"success": False, "error": "Session not found"}

        log = self._get_logger(session_id)
        logger.info(f"[Orchestrator] Continue mapping evaluation result: {result}")
        log.debug(f"Continue mapping evaluation result: all_paths_complete={result.get('all_paths_complete')}",
                  category="ai_response", all_paths_complete=result.get('all_paths_complete'))

        if not result.get("success"):
            logger.warning(f"[Orchestrator] Continue mapping evaluation failed - marking no more paths")
            log.warning("Continue mapping evaluation failed - no more paths", category="ai_response")
            self.transition_to(session_id, MapperState.NO_MORE_PATHS,
                               completed_at=datetime.utcnow().isoformat())
            self._sync_session_status_to_db(session_id, "completed")
            return {"success": True, "state": "no_more_paths", "all_paths_complete": True}

        if result.get("all_paths_complete", True):
            logger.info(f"[Orchestrator] Continue mapping: AI says all paths complete")
            log.info("Continue mapping: all paths already complete", category="milestone")
            self.transition_to(session_id, MapperState.NO_MORE_PATHS,
                               completed_at=datetime.utcnow().isoformat())
            self._sync_session_status_to_db(session_id, "completed")
            return {"success": True, "state": "no_more_paths", "all_paths_complete": True}

        # More paths needed - store junction instructions and start login phase
        ...
        next_path_number = result.get("next_path_number", 1)
        total_paths_estimated = result.get("total_paths_estimated", 1)

        self.update_session(session_id, {
            "junction_instructions": json.dumps(junction_instructions),
            "current_path": next_path_number,
            "total_paths": total_paths_estimated
        })

        logger.info(
            f"[Orchestrator] Continue mapping: starting path {next_path_number}, instructions={junction_instructions}")
        log.info(f"Continue mapping: starting path {next_path_number}", category="milestone",
                 next_path_number=next_path_number, junction_instructions=junction_instructions)

        # Start login phase (existing flow)
        return self.start_login_phase(session_id)
    ...
    def process_celery_result(self, session_id: str, task_name: str, result: Dict) -> Dict:
        ...
        elif task_name == "evaluate_existing_paths":
            return self.handle_continue_mapping_evaluation_result(session_id, result)
